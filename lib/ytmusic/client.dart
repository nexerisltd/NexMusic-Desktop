...
import 'package:nexmusic/ytmusic/helpers.dart';
...
class YTClient {
  ...
  /// If the user has connected their real YouTube Music session (via
  /// [YTMusicAuthService]), adds the Cookie/Authorization headers needed
  /// so requests return their personalized home feed and library instead
  /// of generic/public results. Called before every request since the
  /// SAPISIDHASH auth header is timestamp-based and must be fresh.
  void _applyPersonalAuthIfSignedIn() {
    if (!GetIt.I.isRegistered<YTMusicAuthService>()) {
      debugPrint('[YTAuth] YTMusicAuthService not registered — skipping auth');
      return;
    }
    final auth = GetIt.I<YTMusicAuthService>();
    if (!auth.isSignedIn) {
      debugPrint('[YTAuth] Not signed in — sending unauthenticated request');
      return;
    }

    headers['Cookie'] = auth.cookieHeader!;
    headers['Authorization'] = auth.buildAuthHeader();
    headers['X-Goog-AuthUser'] = '0';
    debugPrint('[YTAuth] Applied personal auth headers to request');
  }
